...
public class SlidingDrawer extends DraggableDrawer {
	...
	protected boolean onMoveAllowDrag(int x, int y, float dx, float dy) {
		switch (getPosition()) {
		case LEFT:
			return (!mMenuVisible && mInitialMotionX <= mTouchSize && (dx > 0)) || (mMenuVisible && x >= mOffsetPixels);
		...
		case TOP:
			return (!mMenuVisible && mInitialMotionY <= mTouchSize && (dy > 0)) || (mMenuVisible && y >= mOffsetPixels);
